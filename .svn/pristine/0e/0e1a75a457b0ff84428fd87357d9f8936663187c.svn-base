...
using EIP.Web.Areas.Quartz.Models;
using Quartz;
using Quartz.Impl.Calendar;
using Quartz.Impl.Matchers;
using Quartz.Impl.Triggers;

namespace EIP.Web.Areas.Quartz.Controllers
{
    //<summary>
    //    基础任务调度控制器
    //</summary>
    public class QuartzController : BaseController
    {
        ...
        public JsonResult PauseAll()
        {
            ...
            try
            {
                RemoteShedulerManager.PauseAll();
                status.ResultSign = ResultSign.Successful;
        ...
        public JsonResult ResumeJob(JobDetailInput input)
        {
            ...
            try
            {
                RemoteShedulerManager.ResumeJob(input.JobName, input.JobGroup);
                status.ResultSign = ResultSign.Successful;
        ...
        public JsonResult CalcRunTime(string cronExpression)
        {
            try
            {
                return Json(RemoteShedulerManager.GetTaskeFireTime(cronExpression, 5));
            }
            catch
            {
                return null;
        ...
        public ViewResultBase CalendarEdit(string calendarName)
        {
            ...
            if (!calendarName.IsNullOrEmpty())
            {
                ...
                var calendar = (CronCalendar)RemoteShedulerManager.GetCalendar(calendarName);
        ...
        public JsonResult DeleteCalendar(string calendarName)
        {
            ...
            try
            {
                if (RemoteShedulerManager.DeleteCalendar(calendarName))
                {
                    status.ResultSign = ResultSign.Successful;
