...
namespace EIP.Common.Dapper
{
    public static class DapperEx
    {
        ...
        public static DateTime GetCurrentDBTime(this DbBase dbs)
        {
            string sql = string.Empty;
            if(dbs.DbType == DBType.Oracle)
            {
                sql = "SELECT SYSDATE DATENOW FROM DUAL";
            }
            else
            {
                sql = "SELECT GETDATE() DATENOW";
            }

            DataTable dtDate = dbs.SqlWithParamsDataTable(sql, "DateNow");
            DateTime dateNow = (DateTime)dtDate.Rows[0][0];
            return dateNow;
        }
