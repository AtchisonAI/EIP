    ...
    $("#list").jgridview(
        {
            ...
            colModel: [
                ...
                { label: "Qtime(Min)", name: "QTIME", width: 110, fixed: true },
                { label: "Htime(Min)", name: "HTIME", width: 110, fixed: true }
            ],
...
function showWipMoveDetail(RECORD_DATE, AREA, CAPABILITY, EQPTYPE) {
    var fromTime =(new Date($('#StartTime').val()).format('yyyy-MM-dd hh:mm:ss'));
    var endTime = (new Date($('#EndTime').val()).format('yyyy-MM-dd hh:mm:ss'));
    ArtDialogOpen("/Reporting/EqpGroupPerformance/WipMoveDetail?DATE=" + RECORD_DATE + "&AREA=" + AREA + "&CAPABILITY=" + CAPABILITY + "&EQPTYPE=" + EQPTYPE + "&StartTime=" + fromTime + "&EndTime=" + endTime, "WIP Detail-" + RECORD_DATE, true, 750, 1200, true);
}