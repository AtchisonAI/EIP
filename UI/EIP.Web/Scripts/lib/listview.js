...
$(function () {
    ...
    $("[name='btn_reset_box']").click(function () {
        ...
        $('.manualfilter:not(.select2)', $selectbox).each(function () {
            $(this).val("");
        });
...
//查询
function search($selectbox) { //获取列表参数
    ...
    if (gridParam.loadonce) {
        ...
        $.extend($grid.getGridParam('postData'), manualFilters);
...
{
    var o = {};
    var a = $('.manualfilter:not(.select2)', $selectbox).serializeArray();
    $.each(a, function() {
