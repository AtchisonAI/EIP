...
function formSubmit() {
    ...
    $.each(ids, function (index, rowid) {
        ...
        if ($(":text[name='Value']", $row).length > 0) {
            var key = $.trim($(":text[name='Key']", $row).val()),
                value = $.trim($(":text[name='Value']", $row).val());
            if (key !== '' && value !='')
                json += "{\"Key\":\"" + key + "\",\"Value\":\"" + value + "\"},";
        } else {

            var rowData = $grid.jqGrid("getRowData", rowid),key = rowData.Key;
            value = rowData.Value;
            if (key != "" && value != "") {
                json += "{\"Key\":\"" + key + "\",\"Value\":\"" + value + "\"},";
            } 
    ...
    UtilAjaxPostWait("/Quartz/Quartz/ScheduleJob",
        submitValue, success);
}
...
function initGrid() {
    ...
    {
        shrinkToFit: true,
            url: "/Quartz/Quartz/GetDetailJobDataMap",
        postData: {
            JobName: $("#JobName").val(),
...
function initChosenCron() {
    ArtDialogOpen("/Quartz/Quartz/Cron", "设置Cron表达式", true, 600, 847);
}
